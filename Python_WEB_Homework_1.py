...
class AddBot(AbstractBot):
    ...
    def handle(self):
        ...
        if record:
            ...
            return f"Added phone {self.phone} to {self.name}"
        else:
            record = Record(self.name)
            record.add_phone(self.phone)
            self.address_book.add_record(record)
            return f"Added new contact {self.name} with phone {self.phone}"
...
class RemoveBot(AbstractBot):
    ...
    def handle(self):
        record = self.address_book.find(self.name)
        if record:
            del self.address_book.records[self.name]
            return f"Removed contact {self.name}"
        else:
            return f"Contact {self.name} not found"

class ChangeBot(AbstractBot):
    def __init__(self, address_book, name, new_phone):
        super().__init__(address_book)
        self.name = name
    ...
    def handle(self):
        record = self.address_book.find(self.name)
        if record:
            record.phones = [self.new_phone]
            return f"Changed phone for {self.name} to {self.new_phone}"
        else:
            return f"Contact {self.name} not found"
